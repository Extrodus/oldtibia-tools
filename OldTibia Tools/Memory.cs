...
namespace TibianicTools
{
    class Memory
    {
        ...
        internal static string ReadString(long address)
        {
            ...
            if (stringRead.Contains("\0")) { return stringRead.Substring(0, stringRead.IndexOf('\0')); }
            return stringRead;
        }
        ...
        /// <returns></returns>
        public static bool WriteRSA(long address, string newKey)
        {
            IntPtr bytesWritten;
            int result;
            ...
            // Write to memory
            result = WinApi.WriteProcessMemory(Client.TibiaHandle, new IntPtr(address), bytes, (uint)bytes.Length, out bytesWritten);

            // Put the protection back on the memory block
            WinApi.VirtualProtectEx(Client.TibiaHandle, new IntPtr(address), new IntPtr(bytes.Length), oldProtection, ref oldProtection);
