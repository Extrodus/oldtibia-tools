...
namespace TibianicTools.Objects
{
    class CharacterList
    {
        ...
        internal static List<CharacterList.Player> GetPlayers()
        {
        ...
        internal static CharacterList.Player GetPlayer(string name)
        {
            foreach (CharacterList.Player player in GetPlayers())
            {
                if (player.Name == name) return player;
            }
            return null;
        }
        internal static CharacterList.Player GetPlayer(int index)
        {
            List<CharacterList.Player> players = GetPlayers();
            if (players.Count <= index || index < 0) return null;
